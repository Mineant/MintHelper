...
namespace MioHelper
{
    ...
    [InitializeOnLoad]
    public class MioFetchCore
    {
        public static MioFetchCore Instance;
        ...
        static MioFetchCore()
        {
            if (Instance == null)
            {
                Instance = new MioFetchCore();
            }
            EditorApplication.update += Instance.OnEditorUpdate;
        }

        /// <summary>
        /// Just a callback that attaches to EditorApplication.Update, so that MioFetch can automatically fetch files even if the editor window isn't open
        /// </summary>
        void OnEditorUpdate()
        {
            if (!MioFetchSettings.EnableMioFetch) return;
            if (MioFetchSettings.Instance.autoRefresh && EditorApplication.timeSinceStartup > lastRefreshTime + MioFetchSettings.Instance.refreshInterval)
            {
        ...
        [MenuItem("MioHelper/MioFetch/Force Refresh All Files")]
        ...
        [MenuItem("MioHelper/MioFetch/Add URLs and Settings...")]
        public static void OpenSettingsAsset()
        {
            EnableMioFetch();
            Selection.activeObject = MioFetchSettings.Instance;
        }

        [MenuItem("MioHelper/MioFetch/Activate/Enable MioFetch")]
        public static void EnableMioFetch()
        {
            if (MioFetchSettings.EnableMioFetch) return;

            Debug.Log("MioHelper/MioFetch is being enabled.");
            MioFetchSettings.EnableMioFetch = true;
        }

        [MenuItem("MioHelper/MioFetch/Activate/Disable MioFetch")]
        public static void DisableMioFetch()
        {
            if (!MioFetchSettings.EnableMioFetch) return;

            Debug.Log("MioFetch is being disabled.");
            MioFetchSettings.EnableMioFetch = false;
        }

        [MenuItem("MioHelper/MioFetch/About", priority = 0)]
        public static void About()
        {
            Debug.Log($"MioFetch is a MioHelper editor tool that automatically downloads files into your Unity project — typically Google Sheets, Google Docs, and Google Drive images.\nGo to 'MioHelper/MioFetch/Activate/Enable MioFetch' to start using MioFetch. This generates a MioFetchSettings asset, where you can configure URLs and file destinations.");
        }
        ...
        {
            results = new UnityWebRequest[MioFetchSettings.Instance.paths.Count];
            refreshInProgress = true;
            lastReport = "MioFetch started refresh at " + System.DateTime.Now.ToLongTimeString() + ", log is below:";

            for (int i = 0; i < results.Length; i++)
            {
                var currentPair = MioFetchSettings.Instance.paths[i];
                if (currentPair != null && currentPair.depth > -1) // make sure tree element is !null and not the root element
                {
                    ...
                    {
                        Debug.LogWarningFormat("MioFetch couldn't retrieve file at <{0}> and error message was: {1}", results[i].url, results[i].error);
                    ...
                    else
                    {
                        ...
                        {
                            Debug.LogWarningFormat("MioFetch couldn't retrieve file at <{0}> because the Google Doc didn't have public link sharing enabled", results[i].url);
            ...
            EditorUtility.SetDirty(MioFetchSettings.Instance);
            refreshInProgress = false;
        }
