...
namespace MioHelper
{
    internal class MioFetchTreeView : TreeViewWithTreeModel<MioFetchPath>
    {
        ...
        public MioFetchTreeView(TreeViewState state, MultiColumnHeader multicolumnHeader, TreeModel<MioFetchPath> model) : base(state, multicolumnHeader, model)
        {
        ...
        void SortByMultipleColumns()
        {
            ...
            var myTypes = rootItem.children.Cast<TreeViewItem<MioFetchPath>>();
        ...
        IOrderedEnumerable<TreeViewItem<MioFetchPath>> InitialOrder(IEnumerable<TreeViewItem<MioFetchPath>> myTypes, int[] history)
        {
        ...
        {
            var item = (TreeViewItem<MioFetchPath>)args.item;
        ...
        void CellGUI(Rect cellRect, TreeViewItem<MioFetchPath> item, MyColumns column, ref RowGUIArgs args)
        {
                ...
                case MyColumns.Name:
                    {
                        ...
                        {
                            if (EditorUtility.DisplayDialog("MioFetch: confirm deletion", "Really delete " + item.data.name + "?", "Yes, delete", "No, cancel"))
                            {
                                var list = new List<MioFetchPath>();
                                list.Add(item.data);
                ...
                case MyColumns.Value3:
                    {
                        ...
                        if (column == MyColumns.Value1)
                        {
                            ...
                            if ( hasURL ) {
                                cellRect.x += cellRect.width;
                                cellRect.width = 20;
                                if ( GUI.Button( cellRect, new GUIContent(">", "click to view in web browser: " + MioFetchCore.UnfixURL(item.data.url) ) ) ) {
                                    Application.OpenURL( MioFetchCore.UnfixURL(item.data.url) );
                                }
                            }
                        ...
                        {
                            item.data.fileType = (MioHelper.FileType)EditorGUI.EnumPopup(cellRect, (Enum)item.data.fileType);
                        }
                        if (column == MyColumns.Value3)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    var newPath = EditorUtility.SaveFilePanelInProject("MioFetch: save " + item.data.name + " URL as file...", item.data.name + "." + item.data.fileType.ToString(), item.data.fileType.ToString(), "Save URL as file...");
        ...
        public static MultiColumnHeaderState CreateDefaultMultiColumnHeaderState(float treeViewWidth)
        {
            var columns = new[]
            {
                ...
                new MultiColumnHeaderState.Column
                {
                    headerContent = new GUIContent("URL", "MioFetch will try to fetch and download content at this URL."),
                    headerTextAlignment = TextAlignment.Left,
                    sortedAscending = true,
                    sortingArrowAlignment = TextAlignment.Right,
                    width = 110,
                ...
                new MultiColumnHeaderState.Column
                {
                    headerContent = new GUIContent("Type", "if you need more file extensions, edit the FileType enum in MioFetchSettings.cs"),
                    headerTextAlignment = TextAlignment.Left,
                    sortedAscending = true,
                    sortingArrowAlignment = TextAlignment.Right,
                    width = 48,
